...
import { useComic, useUpdateComic, useDeleteComic } from "../../hooks/useComics";
...
type EditingField = "title" | "volume_number" | "volume_name" | null;
type ActivePicker = {
  title: string;
...
export default function ComicDetailScreen() {
  ...
  const { imageUri, setImageUri, pickFromGallery, pickFromCamera, clearImage } = useImagePicker();
  ...
  const [authorPickerRole, setAuthorPickerRole] = useState<AuthorRole | null>(null);
  ...
  useEffect(() => {
    ...
    setVolumeNumber(comic.volume_number != null ? String(comic.volume_number) : "");
  ...
  const buildFormData = useCallback(
    ...
    [title, comicType, status, publisherId, storeId, imageUri, seriesId, volumeNumber, volumeName, writerId, artistId, coloristId]
  );
  ...
  const handleChangeCover = () =>
    showPhotoSourcePicker(!!imageUri, async (source) => {
      ...
      else if (source === "remove") { clearImage(); await save({ cover_image_local: null }); }
    });
  ...
  const handlePickType = () =>
    openPicker("Type", COMIC_TYPES.map((t) => ({ label: COMIC_TYPE_LABELS[t], value: t })), comicType,
      async (val) => { setComicType(val); await save({ comic_type: val }); });

  const handlePickPublisher = () =>
    openPicker("Publisher",
      [{ label: "None", value: "" }, ...(publishersList?.map((p) => ({ label: p.name, value: p.id })) ?? [])],
      publisherId,
      async (val) => { setPublisherId(val); await save({ publisher_id: val || null }); });

  const handlePickStore = () =>
    openPicker("Store",
      [{ label: "None", value: "" }, ...(storesList?.map((s) => ({ label: s.name, value: s.id })) ?? [])],
      storeId,
      async (val) => { setStoreId(val); await save({ store_id: val || null }); });

  const handlePickSeries = () =>
    openPicker("Series",
      [{ label: "None", value: "" }, ...(seriesList?.map((s) => ({ label: s.title, value: s.id })) ?? [])],
      seriesId,
      async (val) => { setSeriesId(val); await save({ series_id: val || null }); });
  ...
  const handleAuthorSelected = async (authorId: string) => {
    ...
    if (role === "writer") { setWriterId(authorId); overrides.writer_id = authorId; }
    else if (role === "artist") { setArtistId(authorId); overrides.artist_id = authorId; }
    else if (role === "colorist") { setColoristId(authorId); overrides.colorist_id = authorId; }
  ...
  const handleDelete = () =>
    Alert.alert("Delete Comic", `Delete "${comic?.title}"? This cannot be undone.`, [
      { text: "Cancel", style: "cancel" },
      { text: "Delete", style: "destructive", onPress: async () => { await deleteComic.mutateAsync(id); router.back(); } },
    ]);
  ...
  const coloristName = authorsList?.find((a) => a.id === coloristId)?.name ?? "";
  const typeLabel = COMIC_TYPE_LABELS[comicType as keyof typeof COMIC_TYPE_LABELS] ?? comicType;
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.header}>
          ...
          <View style={styles.coverWrapper}>
            ...
            >
              {imageUri ? (
                <Image source={{ uri: imageUri }} style={styles.cover} contentFit="cover" transition={200} />
              ) : (
                <View style={styles.coverPlaceholder}>
                  <Text style={styles.coverInitial}>{title.charAt(0).toUpperCase()}</Text>
                </View>
              )}
            </Pressable>

            {/* Edit badge on cover */}
            <Pressable onPress={handleChangeCover} style={styles.coverEditBtn} hitSlop={6}>
          ...
          <View style={styles.headerInfo}>
            ...
            {editingField === "title" ? (
              <TextInput
                ...
                onBlur={() => { setEditingField(null); save({ title: title.trim() }); }}
                onSubmitEditing={() => { setEditingField(null); save({ title: title.trim() }); }}
                returnKeyType="done"
                autoFocus
                selectTextOnFocus
              />
            ...
            <Text style={styles.headerMeta} numberOfLines={1}>
              {typeLabel}{publisherName ? ` · ${publisherName}` : ""}
            </Text>
        ...
        <View style={styles.card}>
          ...
          <DetailRow label="Publisher" value={publisherName} onPress={handlePickPublisher} />
          <DetailRow label="Store" value={storeName} onPress={handlePickStore} last />
        </View>

        {/* ── Series ── */}
        <SectionHeader title="Series" />
        <View style={styles.card}>
          <DetailRow label="Series" value={seriesName} onPress={handlePickSeries} />
          {seriesId ? (
            <>
              {editingField === "volume_number" ? (
                <InlineTextRow label="Volume №" value={volumeNumber} onChange={setVolumeNumber}
                  onSave={() => { setEditingField(null); save({ volume_number: volumeNumber.trim() ? parseInt(volumeNumber, 10) : null }); }} numeric />
              ) : (
                <DetailRow label="Volume №" value={volumeNumber} onPress={() => setEditingField("volume_number")} />
              )}
              {editingField === "volume_name" ? (
                <InlineTextRow label="Volume Name" value={volumeName} onChange={setVolumeName}
                  onSave={() => { setEditingField(null); save({ volume_name: volumeName.trim() || null }); }} last />
              ) : (
                <DetailRow label="Volume Name" value={volumeName} onPress={() => setEditingField("volume_name")} last />
              )}
            </>
          ) : (
        ...
        <View style={styles.card}>
          <DetailRow label="Writer" value={writerName} onPress={() => setAuthorPickerRole("writer")} />
          <DetailRow label="Artist" value={artistName} onPress={() => setAuthorPickerRole("artist")} />
          <DetailRow label="Colorist" value={coloristName} onPress={() => setAuthorPickerRole("colorist")} last />
        </View>
